...
class Detail extends Component {
  ...
  async componentDidMount() {
    ...
    const { body: league_divisions } = await Api.get(`admin/sports/leagues/${params.id}/divisions`);

    // const league_divisions_nodes = TreeDataUtil.getTreeFromFlatData({
    //   flatData: league_divisions.map(item => ({
  ...
  render() {
    const {
      league, bet_types, selected_bet_types, league_divisions
    } = this.state;
    return (
      <Page
        fullWidth
      ...
      >
          ...
          <Layout>
            <Layout.Section oneHalf>
              <div style={{ height: '400px' }}>
                {/* <SortableTree
                  treeData={league_divisions_nodes}
                  onChange={() => {}}
                /> */}
              </div>
            </Layout.Section>
        ...
        <Card sectioned title="Teams">
          <Teams
            league={league}
            leagueDivisions={league_divisions}
          />
        </Card>
      </Page>
    );
