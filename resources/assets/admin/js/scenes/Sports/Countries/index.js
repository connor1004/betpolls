...
class Countries extends Component {
  ...
  async search(isSearching = true) {
    ...
    const data = await Api.get('admin/sports/areas', search);
    const {
      response, body
    } = data;
  ...
  async handlePull() {
    await Api.get('admin/sports/areas/pull');
    this.search();
  }

  async handleAdd(values, bags) {
    bags.setSubmitting(true);
    const data = await Api.post('admin/sports/areas', values);
  ...
  async handleEdit(id, values, bags, component) {
    bags.setSubmitting(true);
    const data = await Api.put(`admin/sports/areas/${id}`, values);
    const { response, body } = data;
    switch (response.status) {
      case 422:
        bags.setErrors(body);
  ...
  async handleToggleActive(id) {
    const data = await Api.put(`admin/sports/areas/${id}/toggle-active`);
    const { response } = data;
    switch (response.status) {
      case 422:
        break;
  ...
  async handleDelete(id, component) {
    ...
    const data = await Api.delete(`admin/sports/areas/${id}`);
    const { response } = data;
    switch (response.status) {
      case 422:
        break;
  ...
  async handleDragEnd({ destination, source }) {
    ...
    await Api.put(`admin/sports/areas/${sourceItem.display_order}/${destinationItem.display_order}`);
    this.search(false);
  }
