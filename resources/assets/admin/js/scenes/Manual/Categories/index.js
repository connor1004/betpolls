...
class Categories extends Component {
  ...
  async search(isSearching = true) {
    ...
    const data = await Api.get('admin/manual/categories', search);
    const {
      response, body
    } = data;
  ...
  async handleAdd(values, bags) {
    ...
    const data = await Api.post('admin/manual/categories', values);
  ...
  async handleEdit(id, values, bags, component) {
    bags.setSubmitting(true);
    const data = await Api.put(`admin/manual/categories/${id}`, values);
    const { response, body } = data;
    switch (response.status) {
      case 422:
        bags.setErrors(body);
  ...
  async handleToggleActive(id) {
    const data = await Api.put(`admin/manual/categories/${id}/toggle-active`);
    const { response } = data;
    switch (response.status) {
      case 422:
        break;
  ...
  async handleDelete(id, component) {
    ...
    const data = await Api.delete(`admin/manual/categories/${id}`);
    const { response } = data;
    switch (response.status) {
      case 422:
        break;
  ...
  async handleDragEnd({ destination, source }) {
    ...
    await Api.put(`admin/manual/categories/${sourceItem.display_order}/${destinationItem.display_order}`);
    this.search(false);
  }
