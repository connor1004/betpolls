...
class Countries extends Component {
  ...
  async search(isSearching = true) {
    ...
    const data = await Api.get('admin/manual/countries', search);
    const {
      response, body
    } = data;
  ...
  async handleAdd(values, bags) {
    ...
    const data = await Api.post('admin/manual/countries', values);
  ...
  async handleEdit(id, values, bags, component) {
    bags.setSubmitting(true);
    const data = await Api.put(`admin/manual/countries/${id}`, values);
    const { response, body } = data;
    switch (response.status) {
      case 422:
        bags.setErrors(body);
  ...
  async handleToggleActive(id) {
    const data = await Api.put(`admin/manual/countries/${id}/toggle-active`);
    const { response } = data;
    switch (response.status) {
      case 422:
        break;
  ...
  async handleDelete(id, component) {
    ...
    const data = await Api.delete(`admin/manual/countries/${id}`);
    const { response } = data;
    switch (response.status) {
      case 422:
        break;
  ...
  async handleDragEnd({ destination, source }) {
    ...
    await Api.put(`admin/manual/countries/${sourceItem.display_order}/${destinationItem.display_order}`);
    this.search(false);
  }
